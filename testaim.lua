...
local pflyActive = false
local pflyConn = nil
local pflyKeysConn1 = nil
local pflyKeysConn2 = nil
local selectedBatchPlayers = {}
local showMapNames = false
...
Players.PlayerRemoving:Connect(function(player)
	...
	task.spawn(function()
		...
		if playerStaffRolesCache then
			playerStaffRolesCache[player.UserId] = nil
		end
	end)
...
CMDs[#CMDs + 1] = {NAME = 'freecam / fc [speed]', DESC = 'Allows you to freely move camera around the game (default speed is 1)'}
...
CMDs[#CMDs + 1] = {NAME = 'aimbot / camlock [fov] [smooth]', DESC = 'Enables camera-locking aimbot (defaults: 150 FOV, 0.15 smooth)'}
CMDs[#CMDs + 1] = {NAME = 'unaimbot / nocamlock', DESC = 'Disables the camera-locking aimbot'}
CMDs[#CMDs + 1] = {NAME = 'silentaim [fov]', DESC = 'Enables Silent Aim bullet redirection (defaults: 120 FOV)'}
CMDs[#CMDs + 1] = {NAME = 'unsilentaim / nosilentaim', DESC = 'Disables Silent Aim'}
CMDs[#CMDs + 1] = {NAME = 'aimfov [num]', DESC = 'Sets the Field of View radius for the aimbot'}
CMDs[#CMDs + 1] = {NAME = 'aimkey [key]', DESC = 'Sets the activation key for the aimbot (e.g. rightclick, E, Q)'}
CMDs[#CMDs + 1] = {NAME = 'aimpart [part]', DESC = 'Sets the target body part for the aimbot (Head, Torso, HRP)'}
CMDs[#CMDs + 1] = {NAME = 'aimteam', DESC = 'Toggles aimbot team check'}
CMDs[#CMDs + 1] = {NAME = 'aimwall', DESC = 'Toggles aimbot wall check'}
CMDs[#CMDs + 1] = {NAME = 'aimsmooth [num]', DESC = 'Sets aimbot camera smoothness/lerp (0.01 - 1)'}
CMDs[#CMDs + 1] = {NAME = 'silentfov [num]', DESC = 'Sets the Field of View radius for Silent Aim'}
CMDs[#CMDs + 1] = {NAME = 'silentpart [part]', DESC = 'Sets target body part for Silent Aim (Head, Torso, HRP)'}
CMDs[#CMDs + 1] = {NAME = 'silentteam', DESC = 'Toggles Silent Aim team check'}
CMDs[#CMDs + 1] = {NAME = 'silentwall', DESC = 'Toggles Silent Aim wall check'}
CMDs[#CMDs + 1] = {NAME = 'hitmarkers / hitmarker', DESC = 'Enables damage hit sound effects and temporary red ESP flashes'}
CMDs[#CMDs + 1] = {NAME = 'nohitmarkers / nohitmarker', DESC = 'Disables damage hit sound effects and visual flashes'}
		...
		if plr.Character and not COREGUI:FindFirstChild(plr.Name..'_XOL') then
			...
			if humanoid then
				...
				healthConn = humanoid.HealthChanged:Connect(function(health)
					if health < lastHealth and HitIndicatorsEnabled and plr ~= Players.LocalPlayer then
						local damageDealt = math.floor(lastHealth - health)
						if damageDealt > 0 then
							-- 1. Play hitmarker sound
							pcall(function()
								local hitSound = Instance.new("Sound")
								hitSound.SoundId = "rbxassetid://5043530761"
								hitSound.Volume = 2
							...
							-- 3. Floating 3D text popups
							task.spawn(function()
								local head = plr.Character:FindFirstChild("Head") or getRoot(plr.Character)
								if head then
									local popupBbg = Instance.new("BillboardGui")
									popupBbg.Name = "DamagePopup"
									popupBbg.Size = UDim2.new(0, 100, 0, 40)
									popupBbg.AlwaysOnTop = true
									popupBbg.Adornee = head
									popupBbg.Parent = XOLholder
									...
									label.TextStrokeColor3 = Color3.new(0, 0, 0)
									label.TextStrokeTransparency = 0
									label.Text = tostring(damageDealt)
									label.Parent = popupBbg
									
									-- Float up & Fade out animation
									local tweenInfo = TweenInfo.new(0.6, Enum.EasingStyle.OutQuad)
									local offsetTween = TweenService:Create(popupBbg, tweenInfo, {StudsOffset = Vector3.new(math.random(-2, 2), 4 + math.random(1, 2), math.random(-2, 2))})
									local transparencyTween = TweenService:Create(label, tweenInfo, {TextTransparency = 1, TextStrokeTransparency = 1})
...
function NPC(model)
	...
	task.spawn(function()
		...
		if not COREGUI:FindFirstChild(folderName) and model:IsDescendantOf(workspace) then
			...
			local lastHealth = humanoid.Health
			healthConn = humanoid.HealthChanged:Connect(function(health)
				if health < lastHealth and HitIndicatorsEnabled then
					local damageDealt = math.floor(lastHealth - health)
					if damageDealt > 0 then
						-- 1. Play hitmarker sound
						pcall(function()
	...
	local function startTracking()
		...
		if Drawing then
			pcall(function()
				tracer = Drawing.new("Line")
				tracer.Thickness = 1.5
				tracer.Color = Color3.fromRGB(100, 200, 255)
				tracer.Visible = false
			end)
		end
		...
		if isStaff then
			char.ChildAdded:Connect(function(child)
				if child:IsA("Tool") then
					local rColor = getRoleColor(role or "Staff")
					addStaffLog("⚔️ <b><font color=\"" .. rColor .. "\">" .. player.DisplayName .. "</font></b> equipped tool: " .. child.Name)
				end
			end)
		end
	end)
...
local function createViewHUD(targetPlayer)
	...
	nameLabel.Text = "Viewing: " .. targetPlayer.DisplayName
...
updateStaffListUI = function(excludingPlayer)
	...
	if #staffMembers == 0 then
		...
		for _, data in pairs(staffMembers) do
			...
			row.RichText = true
			row.Text = "• <b>" .. data.Player.DisplayName .. "</b> (" .. data.Player.Name .. ") (" .. data.Role .. ")"
...
getCachedStaffRole = function(player)
	if not player then return false, nil end
	local userId = player.UserId
	if playerStaffRolesCache[userId] ~= nil then
...
addcmd("tmp", {}, function(args, speaker)
	...
	task.spawn(function()
		for _, player in ipairs(checking) do
			getCachedStaffRole(player)
			task.wait(0.2) -- Space out requests to avoid rate limits
		end
		
		task.wait(2) -- Wait for pending checks to finish
		
		for _, player in ipairs(checking) do
			local isStaff, role = getCachedStaffRole(player)
			if isStaff and role then
				table.insert(found, tostring(player.Name) .. " (" .. role .. ")")
			end
		end
...
addcmd("tmpleave", {}, function(args, speaker)
	StaffRolewatchData.Leave = not StaffRolewatchData.Leave
	notify("Staff Watch", StaffRolewatchData.Leave and "Leave has been Enabled" or "Leave has been Disabled")
end)

local function toggleChatSpy(state)
	chatSpyActive = state
	if not state then
		if chatSpyConnection then
			pcall(function() chatSpyConnection:Disconnect() end)
	...
	local function handleMessage(senderName, msg, channelName)
		if not chatSpyActive then return end
		if senderName == Players.LocalPlayer.Name then return end
		...
		if isWhisper or isTeam then
			local chType = isWhisper and "WHISPER" or "TEAM"
			local displayText = "<b><font color=\"rgb(255, 100, 100)\">[" .. chType .. "]</font> @" .. senderName .. ":</b> " .. msg
			addStaffLog(displayText)
		end
	end
	...
	if chatEvents then
		...
		if onMsg and onMsg:IsA("RemoteEvent") then
			chatSpyConnection = onMsg.OnClientEvent:Connect(function(messageData)
				if messageData and messageData.FromSpeaker and messageData.Message then
					handleMessage(messageData.FromSpeaker, messageData.Message, messageData.OriginalChannel or "All")
				end
			end)
		end
	end
	...
	if TextChatService.ChatVersion == Enum.ChatVersion.TextChatService then
		chatSpyConnection = TextChatService.MessageReceived:Connect(function(textChatMessage)
			local sender = textChatMessage.TextSource
			local channel = textChatMessage.TextChannel
			if sender and channel then
				handleMessage(sender.Name, textChatMessage.Text, channel.Name)
			end
		end)
	end
end

local function stopPFly()
	pflyActive = false
	if pflyConn then
		pflyConn:Disconnect()
		pflyConn = nil
	end
	if pflyKeysConn1 then
		pflyKeysConn1:Disconnect()
		pflyKeysConn1 = nil
	end
	if pflyKeysConn2 then
		pflyKeysConn2:Disconnect()
		pflyKeysConn2 = nil
	end
...
local function startPFly()
	...
	pflyKeysConn1 = UserInputService.InputBegan:Connect(function(input, processed)
		if processed then return end
		local speed = iyflyspeed or 1
		if input.KeyCode == Enum.KeyCode.W then
			CONTROL.F = speed
		elseif input.KeyCode == Enum.KeyCode.S then
			CONTROL.B = -speed
	...
	pflyKeysConn2 = UserInputService.InputEnded:Connect(function(input, processed)
		if input.KeyCode == Enum.KeyCode.W then
			CONTROL.F = 0
		elseif input.KeyCode == Enum.KeyCode.S then
			CONTROL.B = 0
	...
	pflyConn = RunService.Heartbeat:Connect(function()
		if not pflyActive or not root or not root.Parent or not humanoid or not humanoid.Parent then
			stopPFly()
			return
		end
		...
		if CONTROL.F ~= 0 then moveVel = moveVel + look * CONTROL.F end
		if CONTROL.B ~= 0 then moveVel = moveVel + look * CONTROL.B end
		if CONTROL.L ~= 0 then moveVel = moveVel + right * CONTROL.L end
		if CONTROL.R ~= 0 then moveVel = moveVel + right * CONTROL.R end
		
		local upSpeed = CONTROL.Q + CONTROL.E
		local targetVel = moveVel * 50 + Vector3.new(0, upSpeed * 50, 0)
...
addcmd("chatspy", {"spy"}, function(args, speaker)
	toggleChatSpy(true)
	notify("Chat Spy", "Chat Spy has been Enabled (logging to ACP Logs)")
end)

addcmd("unchatspy", {"unspy"}, function(args, speaker)
	toggleChatSpy(false)
	notify("Chat Spy", "Chat Spy has been Disabled")
end)

addcmd("pfly", {"physicsfly"}, function(args, speaker)
	startPFly()
	notify("Physics Flight", "Physics Flight has been Enabled")
	if args[1] and isNumber(args[1]) then
		iyflyspeed = tonumber(args[1])
	end
end)

addcmd("unpfly", {"nopfly", "unphysicsfly"}, function(args, speaker)
	stopPFly()
	notify("Physics Flight", "Physics Flight has been Disabled")
end)
...
IY_Connections.IYPlayerAddedConn1 = Players.PlayerAdded:Connect(function(plr)
	...
	task.spawn(function()
		...
		if isStaff then
			local rColor = getRoleColor(role or "Staff")
			addStaffLog("➡️ <b><font color=\"" .. rColor .. "\">" .. plr.DisplayName .. "</font></b> (Staff) joined the server")
...
if not isLegacyChat then
	...
	pcall(function()
		...
		TextChatService.OnIncomingMessage = function(message)
			local properties = originalOnIncomingMessage and originalOnIncomingMessage(message) or Instance.new("TextChatMessageProperties")
	...
	pcall(function()
		local function hookLegacyChatScroller(scroller)
		...
		local function hookLegacyChat()
			...
			local function checkChat(gui)
				if gui.Name == "Chat" then
					local scroller = gui:WaitForChild("Frame", 5)
						and gui.Frame:WaitForChild("ChatChannelParentFrame", 5)
						and gui.Frame.ChatChannelParentFrame:WaitForChild("Frame_MessageLogDisplay", 5)
						and gui.Frame.ChatChannelParentFrame.Frame_MessageLogDisplay:WaitForChild("Scroller", 5)
					if scroller then
						hookLegacyChatScroller(scroller)
					end
				end
			end
...
local function createAdminPortal()
	...
	profileImage.Parent = detailFrame
	...
	alertBellBtn.Font = Enum.Font.GothamBold
	alertBellBtn.TextSize = 14
	alertBellBtn.BorderSizePixel = 0
	alertBellBtn.Visible = false
	
	local bellCorner = Instance.new("UICorner")
	bellCorner.CornerRadius = UDim.new(0, 6)
	bellCorner.Parent = alertBellBtn
	
	local bellStroke = Instance.new("UIStroke")
	bellStroke.Color = Color3.fromRGB(255, 75, 75)
	bellStroke.Thickness = 1.2
	bellStroke.Parent = alertBellBtn
	
	alertBellBtn.MouseEnter:Connect(function()
		alertBellBtn.BackgroundColor3 = Color3.fromRGB(65, 30, 35)
	end)
	alertBellBtn.MouseLeave:Connect(function()
		alertBellBtn.BackgroundColor3 = Color3.fromRGB(45, 20, 25)
	end)
	
	alertBellBtn.MouseButton1Click:Connect(function()
		if selectedPlayer then
			local isStaff, role = getCachedStaffRole(selectedPlayer)
			if isStaff and role then
				createStaffWatchNotification(selectedPlayer, role)
			end
		end
	end)
	
	alertBellBtn.Parent = detailFrame
	...
	batchPanel.Size = UDim2.new(1, 0, 1, 0)
	batchPanel.BackgroundTransparency = 1
	batchPanel.Visible = false
	batchPanel.Parent = detailFrame
	
	local batchTitle = Instance.new("TextLabel")
	batchTitle.Name = "Title"
	batchTitle.Size = UDim2.new(1, -20, 0, 30)
	batchTitle.Position = UDim2.new(0, 10, 0, 10)
	batchTitle.BackgroundTransparency = 1
	batchTitle.Text = "BATCH ACTIONS"
	batchTitle.TextColor3 = Color3.fromRGB(255, 200, 0)
	batchTitle.TextXAlignment = Enum.TextXAlignment.Left
	batchTitle.Font = Enum.Font.GothamBold
	batchTitle.TextSize = 16
	batchTitle.Parent = batchPanel
	
	local batchInfo = Instance.new("TextLabel")
	batchInfo.Name = "Info"
	batchInfo.Size = UDim2.new(1, -20, 0, 60)
	batchInfo.Position = UDim2.new(0, 10, 0, 45)
	batchInfo.BackgroundTransparency = 1
	batchInfo.Text = "0 players selected"
	batchInfo.TextColor3 = Color3.fromRGB(200, 200, 210)
	batchInfo.TextXAlignment = Enum.TextXAlignment.Left
	batchInfo.Font = Enum.Font.Gotham
	batchInfo.TextSize = 14
	batchInfo.TextWrapped = true
	batchInfo.RichText = true
	batchInfo.Parent = batchPanel
	
	local batchBtnGrid = Instance.new("Frame")
	batchBtnGrid.Name = "Buttons"
	batchBtnGrid.Size = UDim2.new(1, -20, 0, 150)
	batchBtnGrid.Position = UDim2.new(0, 10, 0, 120)
	batchBtnGrid.BackgroundTransparency = 1
	batchBtnGrid.Parent = batchPanel
	
	local batchGridLayout = Instance.new("UIGridLayout")
	batchGridLayout.CellSize = UDim2.new(0, 145, 0, 30)
	batchGridLayout.CellPadding = UDim2.new(0, 10, 0, 8)
	batchGridLayout.Parent = batchBtnGrid
	
	local function createBatchButton(text, onClick)
		local btn = Instance.new("TextButton")
		btn.Size = UDim2.new(0, 145, 0, 30)
		btn.BackgroundColor3 = Color3.fromRGB(45, 20, 30)
		btn.Text = text
		...
		btn.MouseEnter:Connect(function() btn.BackgroundColor3 = Color3.fromRGB(65, 30, 45) end)
		btn.MouseLeave:Connect(function() btn.BackgroundColor3 = Color3.fromRGB(45, 20, 30) end)
		btn.MouseButton1Click:Connect(onClick)
		
		btn.Parent = batchBtnGrid
		return btn
	end
	
	createBatchButton("Freeze Selected", function()
		for _, p in pairs(selectedBatchPlayers) do
			execCmd("freeze " .. p.Name)
		end
	end)
	
	createBatchButton("Unfreeze Selected", function()
		for _, p in pairs(selectedBatchPlayers) do
			execCmd("unfreeze " .. p.Name)
		end
	end)
	
	createBatchButton("Fling Selected", function()
		for _, p in pairs(selectedBatchPlayers) do
			execCmd("fling " .. p.Name)
		end
	end)
	
	createBatchButton("Bring Selected", function()
		for _, p in pairs(selectedBatchPlayers) do
			execCmd("cbring " .. p.Name)
		end
	end)
	
	local carouselActive = false
	local carouselThread = nil
	local carouselBtn
	carouselBtn = createBatchButton("View Carousel", function()
		if carouselActive then
			carouselActive = false
			if carouselThread then
				pcall(function() task.cancel(carouselThread) end)
				carouselThread = nil
			end
			execCmd("unview")
			carouselBtn.Text = "View Carousel"
			carouselBtn.TextColor3 = Color3.fromRGB(255, 150, 150)
			carouselBtn.BackgroundColor3 = Color3.fromRGB(45, 20, 30)
		else
			local pList = {}
			for _, p in pairs(selectedBatchPlayers) do
			...
			carouselActive = true
			carouselBtn.Text = "Stop Carousel"
			carouselBtn.TextColor3 = Color3.fromRGB(150, 255, 150)
			carouselBtn.BackgroundColor3 = Color3.fromRGB(20, 45, 30)
			
			carouselThread = task.spawn(function()
				local idx = 1
	...
	createBatchButton("Clear Selection", function()
		if carouselActive then
			carouselActive = false
			if carouselThread then
				pcall(function() task.cancel(carouselThread) end)
		...
		updateList()
		updateDetailFrameForBatch()
	end)
	
	updateDetailFrameForBatch = function()
		local count = 0
		local names = {}
		for _, p in pairs(selectedBatchPlayers) do
		...
		if count > 0 then
			...
			batchInfo.Text = "<b>" .. count .. " players selected:</b>\n" .. table.concat(names, ", ")
	...
	local function selectPlayer(p)
		...
		local function updatePortalButtons()
			...
			if viewing == p then
				...
				portalViewBtn.BackgroundColor3 = Color3.fromRGB(35, 30, 50)
			...
			if isTracking then
				...
				portalTrackBtn.BackgroundColor3 = Color3.fromRGB(35, 30, 50)
				portalTrackBtn.TextColor3 = Color3.fromRGB(200, 180, 255)
			end
		end
		...
		local ageText = "<b>Unknown</b>"
		local ageNum = tonumber(age)
		if ageNum then
			local yearsVal = ageNum / 365.25
			ageText = "<b>" .. ageNum .. " days</b> (<b>" .. string.format("%.1f", yearsVal) .. " years</b>)"
		end
		
		local invisStatusText = "<b>No</b>"
		local parts = 0
		...
		if p.Character then
			for _, child in pairs(p.Character:GetDescendants()) do
				if child:IsA("BasePart") and child.Name ~= "HumanoidRootPart" then
					parts = parts + 1
					if child.Transparency >= 0.9 then
	...
	local function updateList()
		...
		for index, p in ipairs(players) do
			...
			local text = displayName .. " (<b>@" .. p.Name .. "</b>)"
			...
			avatar.Position = UDim2.new(0, -35, 0.5, -13)
			...
			chk.Position = UDim2.new(1, 0, 0.5, -9)
			chk.BackgroundColor3 = Color3.fromRGB(30, 30, 40)
			chk.BorderSizePixel = 0
			chk.Text = selectedBatchPlayers[p.UserId] and "✓" or ""
			chk.TextColor3 = Color3.fromRGB(255, 200, 0)
			...
			chkStroke.Color = selectedBatchPlayers[p.UserId] and Color3.fromRGB(255, 200, 0) or Color3.fromRGB(60, 60, 75)
			chkStroke.Thickness = 1
			chkStroke.Parent = chk
			
			chk.MouseButton1Click:Connect(function()
				if selectedBatchPlayers[p.UserId] then
					selectedBatchPlayers[p.UserId] = nil
					chk.Text = ""
					chkStroke.Color = Color3.fromRGB(60, 60, 75)
				else
					selectedBatchPlayers[p.UserId] = p
					chk.Text = "✓"
					chkStroke.Color = Color3.fromRGB(255, 200, 0)
				end
				if updateDetailFrameForBatch then
					updateDetailFrameForBatch()
				end
			end)
			...
			task.spawn(function()
				...
				if isStaffAsync and not isStaff then
					...
					for _, b in ipairs(badges) do
						if string.find(b, "%[STAFF%]") then
							hasStaffBadge = true
							break
						end
					end
					if not hasStaffBadge then
						table.insert(badges, 1, "<b><font color=\"rgb(255, 200, 0)\">[STAFF]</font></b>")
						pBtn.Text = table.concat(badges, " ") .. " " .. text
					end
				end
	...
	mapViewFrame.Size = UDim2.new(1, -20, 1, -60)
	mapViewFrame.Position = UDim2.new(0, 10, 0, 50)
	mapViewFrame.BackgroundTransparency = 1
	mapViewFrame.Visible = false
	mapViewFrame.Parent = main
	...
	mapStroke.Color = Color3.fromRGB(45, 45, 55)
	mapStroke.Thickness = 1
	mapStroke.Parent = mapContainer
	
	-- Center Dot representing Local Player
	local centerDot = Instance.new("Frame")
	centerDot.Name = "LocalDot"
	centerDot.Size = UDim2.new(0, 10, 0, 10)
	centerDot.Position = UDim2.new(0.5, -5, 0.5, -5)
	centerDot.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
	centerDot.BorderSizePixel = 0
	centerDot.ZIndex = 5
	centerDot.Parent = mapContainer
	...
	centerStroke.Color = Color3.fromRGB(120, 80, 255)
	...
	mapTitle.Text = "🗺️ TACTICAL SATELLITE MAP"
	mapTitle.TextColor3 = Color3.fromRGB(160, 130, 255)
	mapTitle.TextXAlignment = Enum.TextXAlignment.Left
	mapTitle.Font = Enum.Font.GothamBold
	mapTitle.TextSize = 16
	mapTitle.Parent = mapControls
	
	local mapRangeLabel = Instance.new("TextLabel")
	mapRangeLabel.Name = "RangeLabel"
	mapRangeLabel.Size = UDim2.new(1, 0, 0, 25)
	mapRangeLabel.Position = UDim2.new(0, 0, 0, 35)
	mapRangeLabel.BackgroundTransparency = 1
	mapRangeLabel.Text = "Zoom Range: 500 studs"
	mapRangeLabel.TextColor3 = Color3.fromRGB(200, 200, 210)
	mapRangeLabel.TextXAlignment = Enum.TextXAlignment.Left
	mapRangeLabel.Font = Enum.Font.GothamMedium
	mapRangeLabel.TextSize = 13
	mapRangeLabel.Parent = mapControls
	
	local btnZoomIn = Instance.new("TextButton")
	btnZoomIn.Size = UDim2.new(0, 100, 0, 30)
	btnZoomIn.Position = UDim2.new(0, 0, 0, 70)
	btnZoomIn.BackgroundColor3 = Color3.fromRGB(35, 30, 50)
	btnZoomIn.Text = "Zoom In (+)"
	btnZoomIn.TextColor3 = Color3.fromRGB(200, 180, 255)
	btnZoomIn.Font = Enum.Font.GothamMedium
	btnZoomIn.TextSize = 12
	btnZoomIn.BorderSizePixel = 0
	local ziCorner = Instance.new("UICorner")
	ziCorner.CornerRadius = UDim.new(0, 6)
	ziCorner.Parent = btnZoomIn
	btnZoomIn.Parent = mapControls
	
	local btnZoomOut = Instance.new("TextButton")
	btnZoomOut.Size = UDim2.new(0, 100, 0, 30)
	btnZoomOut.Position = UDim2.new(0, 110, 0, 70)
	btnZoomOut.BackgroundColor3 = Color3.fromRGB(35, 30, 50)
	btnZoomOut.Text = "Zoom Out (-)"
	btnZoomOut.TextColor3 = Color3.fromRGB(200, 180, 255)
	btnZoomOut.Font = Enum.Font.GothamMedium
	btnZoomOut.TextSize = 12
	btnZoomOut.BorderSizePixel = 0
	local zoCorner = Instance.new("UICorner")
	zoCorner.CornerRadius = UDim.new(0, 6)
	zoCorner.Parent = btnZoomOut
	btnZoomOut.Parent = mapControls
	
	local nameToggleBtn = Instance.new("TextButton")
	nameToggleBtn.Size = UDim2.new(0, 210, 0, 30)
	nameToggleBtn.Position = UDim2.new(0, 0, 0, 110)
	nameToggleBtn.BackgroundColor3 = Color3.fromRGB(25, 25, 35)
	nameToggleBtn.Text = "Show Player Names: OFF"
	nameToggleBtn.TextColor3 = Color3.fromRGB(150, 150, 160)
	nameToggleBtn.Font = Enum.Font.GothamMedium
	nameToggleBtn.TextSize = 12
	nameToggleBtn.BorderSizePixel = 0
	local ntCorner = Instance.new("UICorner")
	ntCorner.CornerRadius = UDim.new(0, 6)
	ntCorner.Parent = nameToggleBtn
	nameToggleBtn.Parent = mapControls
	
	local selectionLabel = Instance.new("TextLabel")
	selectionLabel.Name = "SelectionLabel"
	selectionLabel.Size = UDim2.new(1, 0, 0, 30)
	selectionLabel.Position = UDim2.new(0, 0, 0, 155)
	selectionLabel.BackgroundTransparency = 1
	selectionLabel.Text = "Selected Player: None"
	selectionLabel.TextColor3 = Color3.fromRGB(255, 200, 0)
	selectionLabel.TextXAlignment = Enum.TextXAlignment.Left
	selectionLabel.Font = Enum.Font.GothamBold
	selectionLabel.TextSize = 13
	selectionLabel.RichText = true
	selectionLabel.Parent = mapControls
	
	local btnTeleport = Instance.new("TextButton")
	btnTeleport.Size = UDim2.new(0, 210, 0, 35)
	btnTeleport.Position = UDim2.new(0, 0, 0, 195)
	btnTeleport.BackgroundColor3 = Color3.fromRGB(35, 30, 50)
	btnTeleport.Text = "Teleport to Player"
	btnTeleport.TextColor3 = Color3.fromRGB(200, 180, 255)
	...
	btnTeleport.Parent = mapControls
	
	-- Map Legend
	local mapLegend = Instance.new("TextLabel")
	mapLegend.Name = "Legend"
	mapLegend.Size = UDim2.new(1, 0, 0, 80)
	mapLegend.Position = UDim2.new(0, 0, 0, 240)
	mapLegend.BackgroundTransparency = 1
	mapLegend.Text = "<b>LEGEND:</b>\n" ..
		"<font color=\"rgb(255, 200, 0)\">● Staff Member</font>  " ..
		"<font color=\"rgb(100, 200, 255)\">● Teammate</font>\n" ..
		"<font color=\"rgb(255, 100, 100)\">● Opponent</font>  " ..
		"<font color=\"rgb(220, 220, 220)\">● Neutral/Teamless</font>"
	mapLegend.TextColor3 = Color3.fromRGB(160, 160, 170)
	mapLegend.TextXAlignment = Enum.TextXAlignment.Left
	mapLegend.Font = Enum.Font.Gotham
	mapLegend.TextSize = 12
	mapLegend.TextWrapped = true
	mapLegend.RichText = true
	mapLegend.Parent = mapControls

	btnZoomIn.MouseButton1Click:Connect(function()
		mapRange = math.max(100, math.floor(mapRange * 0.75))
		mapRangeLabel.Text = "Zoom Range: " .. mapRange .. " studs"
	end)
	
	btnZoomOut.MouseButton1Click:Connect(function()
		mapRange = math.min(3000, math.floor(mapRange * 1.25))
		mapRangeLabel.Text = "Zoom Range: " .. mapRange .. " studs"
	end)
	
	nameToggleBtn.MouseButton1Click:Connect(function()
		showMapNames = not showMapNames
		nameToggleBtn.Text = "Show Player Names: " .. (showMapNames and "ON" or "OFF")
		nameToggleBtn.TextColor3 = showMapNames and Color3.fromRGB(200, 180, 255) or Color3.fromRGB(150, 150, 160)
		nameToggleBtn.BackgroundColor3 = showMapNames and Color3.fromRGB(35, 30, 50) or Color3.fromRGB(25, 25, 35)
	end)
	...
	mapConnection = RunService.Heartbeat:Connect(function()
		...
		if camera then
			local _, yaw, _ = camera.CFrame:ToEulerAnglesYXZ()
			centerDot.Rotation = -math.deg(yaw)
		end
		...
		for _, player in pairs(Players:GetPlayers()) do
			if player ~= Players.LocalPlayer and player.Character then
				...
				if root then
					...
					local mapRadius = 150
					local xScaled = (xOffset / mapRange) * mapRadius
					local yScaled = (zOffset / mapRange) * mapRadius
					
					local distScaled = math.sqrt(xScaled^2 + yScaled^2)
					local visible = distScaled <= mapRadius
					
					local dot = MapDots[player]
					local label = MapLabels[player]
					
					if visible then
						if not dot then
							dot = Instance.new("TextButton")
							dot.Name = player.Name .. "_MapDot"
							...
							dot.Parent = mapContainer
							MapDots[player] = dot
							
							dot.MouseButton1Click:Connect(function()
								selectedMapPlayer = player
						...
						local isStaff = false
						if getCachedStaffRole then
							isStaff, _ = getCachedStaffRole(player)
						end
						
						local color = Color3.fromRGB(220, 220, 220)
						if isStaff then
							color = Color3.fromRGB(255, 200, 0)
						elseif player.Team and Players.LocalPlayer.Team then
						...
						local stroke = dot:FindFirstChild("SelectionStroke")
						if selectedMapPlayer == player then
							dot.Size = UDim2.new(0, 12, 0, 12)
							dot.Position = UDim2.new(0.5, xScaled - 6, 0.5, yScaled - 6)
							dot.ZIndex = 4
							if not stroke then
								stroke = Instance.new("UIStroke")
							...
							dot.Size = UDim2.new(0, 8, 0, 8)
							dot.Position = UDim2.new(0.5, xScaled - 4, 0.5, yScaled - 4)
							dot.ZIndex = 3
							if stroke then stroke:Destroy() end
						end
						
						if showMapNames then
							if not label then
								...
								label.TextSize = 8
								label.TextXAlignment = Enum.TextXAlignment.Center
								label.Parent = mapContainer
								MapLabels[player] = label
							end
							label.Text = player.DisplayName or player.Name
							label.Position = UDim2.new(0.5, xScaled - 50, 0.5, yScaled - 16)
							label.Visible = true
						else
		...
		for player, dot in pairs(MapDots) do
			if not activePlayers[player] then
				pcall(function() dot:Destroy() end)
				MapDots[player] = nil
				if MapLabels[player] then
					pcall(function() MapLabels[player]:Destroy() end)
					MapLabels[player] = nil
				end
				if selectedMapPlayer == player then
					selectedMapPlayer = nil
					updateMapSelectionUI()
	...
	createSettingToggle("💬 Server Chat Spy (Whispers/Teams)", function() return chatSpyActive end, toggleChatSpy)
	...
	local function updateLogsUI()
		...
		logsScroll.CanvasSize = UDim2.new(0, 0, 0, #StaffLogs * 32)
	...
	local function setTab(tabName)
		...
		mapViewFrame.Visible = (tabName == "Map")
	...
	mapTabBtn.MouseButton1Click:Connect(function() setTab("Map") end)
...
addcmd("unviewhud", {}, function(args, speaker)
	destroyViewHUD()
	notify("View HUD", "View HUD Disabled")
end)
...
local function expandHitboxes()
	for _, p in pairs(Players:GetPlayers()) do
		if p ~= Players.LocalPlayer and p.Character then
			...
			if hrp then
				if not OriginalHitboxes[p.UserId] then
					OriginalHitboxes[p.UserId] = {
						Size = hrp.Size,
...
local function triggerStaffAlarmEffect()
	if not AlarmFlasher then
		AlarmFlasher = Instance.new("Frame")
		AlarmFlasher.Name = "StaffAlarmFlasher"
		AlarmFlasher.Size = UDim2.new(1, 0, 1, 0)
		AlarmFlasher.BackgroundColor3 = Color3.fromRGB(255, 0, 0)
		AlarmFlasher.BackgroundTransparency = 1
		AlarmFlasher.ZIndex = 999999
...
local function checkStaffProximity()
	local localChar = Players.LocalPlayer.Character
...
local function toggleStaffAlarm(enable)
	...
	if enable then
		...
		StaffAlarmConnection = RunService.Heartbeat:Connect(function()
			if tick() - lastCheck >= 1.5 then
				lastCheck = tick()
				pcall(checkStaffProximity)
			end
		end)
	else
...
local function toggleGhostFling(enable)
	local char = Players.LocalPlayer.Character
	local root = char and char:FindFirstChild("HumanoidRootPart")
	local hum = char and char:FindFirstChildOfClass("Humanoid")
	
	if not enable then
		GhostFlingEnabled = false
		if GhostFlingLoop then GhostFlingLoop:Disconnect() GhostFlingLoop = nil end
		
		if char then
			for _, part in pairs(char:GetDescendants()) do
				if part:IsA("BasePart") then
					part.Transparency = part:GetAttribute("OrigTrans") or 0
					part.CanCollide = part:GetAttribute("OrigCollide") == nil and true or part:GetAttribute("OrigCollide")
		...
		if root then
			...
			if GhostOriginalCFrame then
				root.CFrame = GhostOriginalCFrame
			end
		end
		
		if GhostPlatform then GhostPlatform:Destroy() GhostPlatform = nil end
		notify("Ghost Fling", "Ghost Fling Disabled")
		return
	end
	
	if not char or not root or not hum then
		notify("Ghost Fling", "Character not fully loaded.")
		return
	end
	
	GhostFlingEnabled = true
	GhostOriginalCFrame = root.CFrame
	...
	for _, part in pairs(char:GetDescendants()) do
		if part:IsA("BasePart") then
			...
			if not part:GetAttribute("OrigCollide") then
	...
	GhostPlatform.CFrame = GhostOriginalCFrame * CFrame.new(0, -3.5, 0)
	GhostPlatform.Anchored = true
	GhostPlatform.CanCollide = true
	GhostPlatform.Transparency = 0.5
	GhostPlatform.Color = Color3.fromRGB(120, 80, 255)
	GhostPlatform.Parent = workspace
	
	notify("Ghost Fling", "Ghost Fling Enabled (Invis + Spin active). Go near players to fling them!")
	
	GhostFlingLoop = RunService.Heartbeat:Connect(function()
		if not char or not char.Parent or not root then
			toggleGhostFling(false)
			return
		end
...
-- Waypoints & POI ESP variables
local POIWaypoints = {}
local POIActive = false

local function clearWaypoints()
	for _, wp in pairs(POIWaypoints) do
...
local function createWaypoints()
	...
	local function checkPOI(obj)
		if not POIActive then return end
		...
		if obj:IsA("SpawnLocation") then
			matches = true
			displayName = "Spawn"
		elseif name:find("vault") or name:find("safe") then
			matches = true
			displayName = "Vault/Safe"
		elseif name:find("exit") or name:find("escape") then
			matches = true
			displayName = "Exit"
		elseif name:find("shop") or name:find("store") or name:find("dealer") then
			matches = true
			displayName = "Shop"
		elseif name:find("garage") then
			matches = true
			displayName = "Garage"
		elseif name:find("flag") or name:find("objective") then
			matches = true
			displayName = "Objective"
		end
		
		if matches and obj:IsA("BasePart") then
			local bgui = Instance.new("BillboardGui")
			bgui.Name = "POI_Waypoint"
			bgui.Size = UDim2.new(0, 100, 0, 40)
			bgui.AlwaysOnTop = true
			bgui.Adornee = obj
			bgui.Parent = COREGUI:FindFirstChild("RobloxGui") or COREGUI:FindFirstChildOfClass("ScreenGui") or COREGUI:GetChildren()[1]
			...
			table.insert(POIWaypoints, bgui)
			
			task.spawn(function()
				while bgui and bgui.Parent and POIActive do
					local char = Players.LocalPlayer.Character
					local root = char and char:FindFirstChild("HumanoidRootPart")
					if root and obj and obj.Parent then
						local dist = math.floor((root.Position - obj.Position).Magnitude)
						text.Text = "📍 <b>" .. displayName .. "</b>\n[" .. dist .. "s]"
					else
						break
					end
					task.wait(0.5)
	...
	for _, child in pairs(workspace:GetDescendants()) do
		pcall(checkPOI, child)
	end
	
	notify("Waypoints", "Waypoints & POI ESP enabled")
end

-- Safe-Zone variables
local SafeZoneCoordinate = CFrame.new(0, 5000, 0)
local SafeZonePlatform = nil

local function setSafeZone(cf)
	SafeZoneCoordinate = cf
	notify("Safe-Zone", "Safe-zone set to current location")
end
...
local function teleportToSafeZone()
	...
	local root = char and char:FindFirstChild("HumanoidRootPart")
	if not root then
		notify("Safe-Zone", "Character not loaded")
		return
	end
	...
	if not SafeZonePlatform then
		...
		SafeZonePlatform.CFrame = SafeZoneCoordinate * CFrame.new(0, -3.5, 0)
		SafeZonePlatform.Anchored = true
		SafeZonePlatform.CanCollide = true
		SafeZonePlatform.Material = Enum.Material.Glass
		SafeZonePlatform.Color = Color3.fromRGB(120, 80, 255)
		SafeZonePlatform.Parent = workspace
	end
	
	root.CFrame = SafeZoneCoordinate
	notify("Safe-Zone", "Teleported to safe-zone!")
end
...
local function destroyItemFinderUI()
	...
	if ItemFinderConnection then ItemFinderConnection:Disconnect() ItemFinderConnection = nil end
end

local function createItemFinderUI()
	destroyItemFinderUI()
	
	local frame = Instance.new("Frame")
	frame.Name = "ItemFinderFrame"
	...
	close.TextSize = 14
	close.Parent = header
	close.MouseButton1Click:Connect(destroyItemFinderUI)
	...
	local function updateFinder()
		...
		local items = {}
		for _, child in pairs(workspace:GetDescendants()) do
			if child:IsA("Tool") and not child:IsDescendantOf(Players) then
				local handle = child:FindFirstChild("Handle") or child:FindFirstChildOfClass("BasePart")
				if handle then
					local root = Players.LocalPlayer.Character and Players.LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
					local dist = root and math.floor((root.Position - handle.Position).Magnitude) or 99999
					table.insert(items, {Instance = child, Handle = handle, Distance = dist})
				end
			end
		end
		...
		for _, item in ipairs(items) do
			...
			btn.BackgroundColor3 = Color3.fromRGB(25, 25, 35)
			btn.BorderSizePixel = 0
			btn.Text = "  " .. item.Instance.Name .. " [" .. item.Distance .. " studs]"
			...
			btn.MouseButton1Click:Connect(function()
				local root = Players.LocalPlayer.Character and Players.LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
				if root then
					root.CFrame = item.Handle.CFrame * CFrame.new(0, 1, 0)
					notify("Item Finder", "Teleported to " .. item.Instance.Name)
				end
			end)
	...
	updateFinder()
	local lastRef = tick()
	ItemFinderConnection = RunService.Heartbeat:Connect(function()
		if tick() - lastRef >= 2 then
			lastRef = tick()
			pcall(updateFinder)
		end
	end)
...
local function autoGrabItems()
	local root = Players.LocalPlayer.Character and Players.LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
	if not root then
		notify("Auto Grab", "Character not loaded")
		return
	end
	...
	for _, child in pairs(workspace:GetDescendants()) do
		if child:IsA("Tool") and not child:IsDescendantOf(Players) then
			local handle = child:FindFirstChild("Handle") or child:FindFirstChildOfClass("BasePart")
			if handle then
				table.insert(items, {Instance = child, Handle = handle})
			end
		end
	end
	
	if #items == 0 then
		notify("Auto Grab", "No free tools found in workspace")
		return
	end
	
	notify("Auto Grab", "Grabbing " .. #items .. " items, please wait...")
	task.spawn(function()
		...
		notify("Auto Grab", "Finished grabbing items!")
	end)
end
...
addcmd("grabitems", {"autotool"}, function(args, speaker)
	autoGrabItems()
end)

addcmd("hitbox", {"hb"}, function(args, speaker)
	local size = tonumber(args[1]) or 10
	HitboxSize = size
	startHitboxLoop()
	notify("Hitbox Expander", "Hitboxes expanded to size " .. size)
end)

addcmd("unhitbox", {"nohb"}, function(args, speaker)
	stopHitboxLoop()
	notify("Hitbox Expander", "Hitboxes restored to original size")
end)

addcmd("staffalarm", {"staffproximity"}, function(args, speaker)
	toggleStaffAlarm(true)
end)

addcmd("unstaffalarm", {}, function(args, speaker)
	toggleStaffAlarm(false)
end)

addcmd("ghostfling", {"gfling"}, function(args, speaker)
	toggleGhostFling(true)
end)

addcmd("unghostfling", {"ungfling"}, function(args, speaker)
	toggleGhostFling(false)
end)

addcmd("waypoints", {"poi"}, function(args, speaker)
	createWaypoints()
end)

addcmd("unwaypoints", {"nopoi"}, function(args, speaker)
	clearWaypoints()
	notify("Waypoints", "Waypoints disabled")
end)

addcmd("safezone", {"escape"}, function(args, speaker)
	teleportToSafeZone()
end)

addcmd("setsafezone", {"setescape"}, function(args, speaker)
	local char = Players.LocalPlayer.Character
	local root = char and char:FindFirstChild("HumanoidRootPart")
	if root then
		setSafeZone(root.CFrame)
	else
		notify("Safe-Zone", "Character not loaded")
	end
end)
...
-- =============================================================================
-- PREMIUM CAMERA AIMBOT & SILENT AIM SYSTEM
-- =============================================================================
AimbotEnabled = false
AimbotFOV = 150
...
if Drawing then
	pcall(function()
		...
		AimbotCircle.Color = Color3.fromRGB(0, 255, 204) -- cyber cyan
		...
		SilentAimCircle.Color = Color3.fromRGB(255, 100, 100) -- soft red
...
local function parseKey(keyStr)
	keyStr = keyStr:lower()
	if keyStr == "rightclick" or keyStr == "mouse2" or keyStr == "rmb" then
		return Enum.UserInputType.MouseButton2
	elseif keyStr == "leftclick" or keyStr == "mouse1" or keyStr == "lmb" then
		return Enum.UserInputType.MouseButton1
	else
		for _, key in ipairs(Enum.KeyCode:GetEnumItems()) do
			if key.Name:lower() == keyStr then
				return key
			end
		end
	end
...
local function getAimbotTarget()
	...
	if not camera or not localChar or not localPlayer then return nil end
	
	local mousePos = UserInputService:GetMouseLocation()
	
	for _, plr in ipairs(Players:GetPlayers()) do
		if plr ~= localPlayer and plr.Character and plr.Character:FindFirstChildOfClass("Humanoid") and plr.Character:FindFirstChildOfClass("Humanoid").Health > 0 then
			...
			if not targetPart and AimbotPart == "Torso" then
				targetPart = plr.Character:FindFirstChild("UpperTorso") or plr.Character:FindFirstChild("HumanoidRootPart")
			end
			...
			if targetPart then
				...
				if onScreen then
					...
					if distanceToMouse <= AimbotFOV and distanceToMouse < shortestDistance then
						local canSee = true
						if AimbotWallCheck then
							pcall(function()
								...
								if rayResult then
									...
									if hitPart and hitPart.CanCollide and hitPart.Transparency <= 0.9 then
										canSee = false
									end
								end
							end)
...
local function getSilentAimTarget()
	...
	for _, plr in ipairs(Players:GetPlayers()) do
		if plr ~= localPlayer and plr.Character and plr.Character:FindFirstChildOfClass("Humanoid") and plr.Character:FindFirstChildOfClass("Humanoid").Health > 0 then
			...
			if SilentAimTeamCheck and plr.Team == localPlayer.Team then
				continue
			end
			
			local targetPart = plr.Character:FindFirstChild(SilentAimPart)
			if not targetPart and SilentAimPart == "Torso" then
				targetPart = plr.Character:FindFirstChild("UpperTorso") or plr.Character:FindFirstChild("HumanoidRootPart")
			end
			...
			if targetPart then
				...
				if onScreen then
					...
					if distanceToMouse <= SilentAimFOV and distanceToMouse < shortestDistance then
						local canSee = true
...
local function aimbotUpdate()
	...
	if SilentAimCircle then
		if SilentAimEnabled and UserInputService:GetFocusedTextBox() == nil then
	...
	if AimbotEnabled and isHoldingKey and camera and UserInputService:GetFocusedTextBox() == nil then
		targetPlayer = getAimbotTarget()
		if targetPlayer and targetPlayer.Character then
			local targetPart = targetPlayer.Character:FindFirstChild(AimbotPart)
			if not targetPart and AimbotPart == "Torso" then
				targetPart = targetPlayer.Character:FindFirstChild("UpperTorso") or targetPlayer.Character:FindFirstChild("HumanoidRootPart")
			end
			...
			if targetPart then
				...
				camera.CFrame = currentCFrame:Lerp(lookAtCFrame, AimbotSmoothness)
			end
		end
	else
		targetPlayer = nil
...
-- Hook client mouse metamethod for Silent Aim
local Mouse = Players.LocalPlayer:GetMouse()
local indexHook
if hookmetamethod then
	pcall(function()
		indexHook = hookmetamethod(game, "__index", function(self, index)
			if self == Mouse and (index == "Hit" or index == "Target") and SilentAimEnabled then
				local targetPart = nil
				pcall(function()
					local target = getSilentAimTarget()
					if target and target.Character then
						targetPart = target.Character:FindFirstChild(SilentAimPart)
						if not targetPart and SilentAimPart == "Torso" then
							targetPart = target.Character:FindFirstChild("UpperTorso") or target.Character:FindFirstChild("HumanoidRootPart")
						end
					end
				end)
				if targetPart then
	...
	warn("Executor does not support hookmetamethod. Silent Aim will be unavailable.")
end

-- Main Commands
addcmd("aimbot", {"camlock"}, function(args, speaker)
	AimbotEnabled = true
	if args[1] then
		AimbotFOV = tonumber(args[1]) or AimbotFOV
	end
	if args[2] then
		AimbotSmoothness = math.clamp(tonumber(args[2]) or AimbotSmoothness, 0.01, 1)
	end
	notify("Aimbot", string.format("Aimbot Enabled. FOV: %d, Smoothness: %.2f. Hold right-click to lock.", AimbotFOV, AimbotSmoothness))
end)

addcmd("unaimbot", {"nocamlock"}, function(args, speaker)
	AimbotEnabled = false
	if AimbotCircle then AimbotCircle.Visible = false end
	notify("Aimbot", "Aimbot/Camlock Disabled.")
end)

addcmd("silentaim", {}, function(args, speaker)
	SilentAimEnabled = true
	if args[1] then
		SilentAimFOV = tonumber(args[1]) or SilentAimFOV
	end
	notify("Silent Aim", string.format("Silent Aim Enabled. FOV: %d. Always active when shooting.", SilentAimFOV))
end)

addcmd("unsilentaim", {"nosilentaim"}, function(args, speaker)
	SilentAimEnabled = false
	if SilentAimCircle then SilentAimCircle.Visible = false end
	notify("Silent Aim", "Silent Aim Disabled.")
end)

-- Configurations (Aimbot)
addcmd("aimfov", {}, function(args, speaker)
	local num = tonumber(args[1])
	if num then
		AimbotFOV = num
		notify("Aimbot", "Aimbot FOV set to " .. tostring(AimbotFOV))
	else
		notify("Aimbot", "Current FOV: " .. tostring(AimbotFOV))
	end
end)

addcmd("aimkey", {}, function(args, speaker)
	if not args[1] then 
		notify("Aimbot", "Current key: " .. tostring(AimbotKey.Name or AimbotKey))
		return 
	end
	local key = parseKey(args[1])
	if key then
		AimbotKey = key
		
		-- Reconnect input listeners with the new key
		...
		notify("Aimbot", "Aimbot activation key set to " .. tostring(AimbotKey.Name or AimbotKey))
	else
		notify("Aimbot", "Invalid key. Use rightclick/rmb, leftclick/lmb, or keyboard keys like Q, E, F.")
	end
end)

addcmd("aimpart", {}, function(args, speaker)
	local part = args[1]
	if part then
		part = part:lower()
		if part == "head" then
			AimbotPart = "Head"
		elseif part == "torso" or part == "loweritorso" or part == "upperitorso" then
			AimbotPart = "Torso"
		elseif part == "root" or part == "hrp" or part == "humanoidrootpart" then
			AimbotPart = "HumanoidRootPart"
		else
			notify("Aimbot", "Invalid part. Use Head, Torso, or HumanoidRootPart.")
			return
		end
		notify("Aimbot", "Aimbot target part set to " .. AimbotPart)
	else
		notify("Aimbot", "Current target part: " .. AimbotPart)
	end
end)

addcmd("aimteam", {}, function(args, speaker)
	AimbotTeamCheck = not AimbotTeamCheck
	notify("Aimbot", "Team Check set to " .. tostring(AimbotTeamCheck))
end)

addcmd("aimwall", {}, function(args, speaker)
	AimbotWallCheck = not AimbotWallCheck
	notify("Aimbot", "Wall/Occlusion Check set to " .. tostring(AimbotWallCheck))
end)

addcmd("aimsmooth", {}, function(args, speaker)
	local num = tonumber(args[1])
	if num then
		AimbotSmoothness = math.clamp(num, 0.01, 1)
		notify("Aimbot", "Aimbot Smoothness set to " .. tostring(AimbotSmoothness))
	else
		notify("Aimbot", "Current Smoothness: " .. tostring(AimbotSmoothness))
	end
end)

-- Configurations (Silent Aim)
addcmd("silentfov", {}, function(args, speaker)
	local num = tonumber(args[1])
	if num then
		SilentAimFOV = num
		notify("Silent Aim", "Silent Aim FOV set to " .. tostring(SilentAimFOV))
	else
		notify("Silent Aim", "Current FOV: " .. tostring(SilentAimFOV))
	end
end)

addcmd("silentpart", {}, function(args, speaker)
	local part = args[1]
	if part then
		part = part:lower()
		if part == "head" then
		...
		elseif part == "root" or part == "hrp" or part == "humanoidrootpart" then
			...
			notify("Silent Aim", "Invalid part. Use Head, Torso, or HumanoidRootPart.")
			return
		end
		notify("Silent Aim", "Silent Aim target part set to " .. SilentAimPart)
	else
		notify("Silent Aim", "Current target part: " .. SilentAimPart)
	end
end)

addcmd("silentteam", {}, function(args, speaker)
	SilentAimTeamCheck = not SilentAimTeamCheck
	notify("Silent Aim", "Team Check set to " .. tostring(SilentAimTeamCheck))
end)

addcmd("silentwall", {}, function(args, speaker)
	SilentAimWallCheck = not SilentAimWallCheck
	notify("Silent Aim", "Wall/Occlusion Check set to " .. tostring(SilentAimWallCheck))
end)

addcmd("hitmarkers", {"hitmarker", "hitfeedback"}, function(args, speaker)
	HitIndicatorsEnabled = true
	notify("Hit Indicators", "Hitmarkers and damage indicators have been enabled.")
end)

addcmd("nohitmarkers", {"nohitmarker", "nohitfeedback"}, function(args, speaker)
	HitIndicatorsEnabled = false
	notify("Hit Indicators", "Hitmarkers and damage indicators have been disabled.")
end)